...
*令牌桶 + 三态熔断器 + Redis+Lua 分布式计数限流*

#### 核心技术点

Redis + Lua 分布式令牌桶限流（AOP 注解 `@RateLimit`，所有实例共用一个桶）+ 自研三态熔断器（CLOSED → OPEN → HALF_OPEN）+ Redis + Lua 的分布式计数限流（`SmsRateLimiter`，按手机号冷却 / 日上限 / 全站兜底三道闸门）。滑动窗口统计失败率，CAS 保证状态转换原子性，半开状态放少量请求探测恢复。
...
- **两种限流器的分工是「维度」，不是「单机 vs 分布式」**：`@RateLimit`（全站令牌桶）限的是**整个接口的容量**——保护后端不被压垮，阈值应从压测测出的容量推导；`SmsRateLimiter`（按手机号计数）限的是**单个业务主体**的行为——防刷防枚举，阈值来自业务规则与成本预算。**两者都放在 Redis 里跨实例共享**，早期把前者做成 JVM 进程内 Guava RateLimiter 是踩过的坑：它的阈值随实例数放大、重启后桶还是满的。用错了维度同样是坑——验证码接口挂 `qps=1`，结果 2 个不同用户在同一秒就互相冲突
- **限流状态为什么必须在 Redis**：无论限的是接口容量还是单个用户，阈值都必须是**全局**的。任何进程内状态（Guava RateLimiter、本地计数器）都会让阈值随实例数放大，而且实例重启就归零——这两点都是静默的，监控上看不出来
- **check-and-set 必须走 Lua**："查冷却键"和"写冷却键"分两次调用就是 check-then-act 竞态——两个并发请求可能都查到"不存在"，然后都放行。同理"读计数判断超限"和"自增计数"。Lua 在 Redis 里串行执行，把多步焊成一个原子整体
- **窗口选滚动还是自然日**：滚动窗口（TTL 从首次请求起算）不需要选业务时区，且比自然日更严——自然日允许攻击者在 23:59 和 00:01 各发满额度，两分钟打满双倍
- **fail-open 还是 fail-closed 的判据**：看"降级的代价是丢钱还是丢防护"。秒杀链路 Redis 挂了放行会超卖（丢钱）→ 失败关闭；验证码接口 Redis 挂了拒绝会让全站登不上（丢可用性）→ 放行
- **只配 fail-open 是不够的，客户端超时才是前提**：本项目 `spring.redis.timeout` 原先没配，用的是 Lettuce 默认的 60 秒。实测停掉 Redis 后请求挂 >20 秒无任何响应，fail-open 代码是对的但根本触发不了——降级速度以分钟计就等于没有降级
