...
Guava RateLimiter 令牌桶限流（AOP 注解 `@RateLimit`）+ 自研三态熔断器（CLOSED → OPEN → HALF_OPEN）+ Redis + Lua 的分布式计数限流（`SmsRateLimiter`，按手机号冷却 / 日上限 / 全站兜底三道闸门）。滑动窗口统计失败率，CAS 保证状态转换原子性，半开状态放少量请求探测恢复。
...
- **两种限流器的分工不能混**：`@RateLimit`（Guava 令牌桶）限的是**单机、全站维度**的容量——保护后端不被压垮，阈值应从压测容量推导；`SmsRateLimiter`（Redis + Lua 计数）限的是**跨实例、按业务主体**的行为——防刷防枚举，阈值来自业务规则与成本预算。用前者去做后者就是本项目踩过的坑：验证码接口挂 `qps=1`，结果 2 个不同用户在同一秒就互相冲突
- **防刷阈值必须用 Redis**：Guava 限流器存在 JVM 内存里，部署 N 个实例阈值就放大 N 倍。而防刷阈值必须是全局的，所以状态只能在 Redis
