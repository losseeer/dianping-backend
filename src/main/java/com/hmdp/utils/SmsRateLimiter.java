 ...
 * <p>
 * 【八股：为什么不用 @RateLimit？】
 * 1. 维度不对：@RateLimit 限的是"整个接口每秒 N 次"，意味着 2 个不同用户
 *    在同一秒就会互相冲突、5 个攻击者就能让全站用户收不到验证码。这个接口
 *    要限的是"同一个手机号发得太快"，维度是手机号，不是全站。
 *    （注意这里说的是<strong>维度</strong>。@RateLimit 早期用的是 JVM 进程内的
 *    Guava RateLimiter，阈值确实会随实例数放大，但它现在已经换成 Redis + Lua
 *    实现，那条理由不再成立——用错的只剩维度这一条。）
 * 2. 挡不住手机号枚举：验证码接口的真实攻击是"用大量不同手机号轰炸"
