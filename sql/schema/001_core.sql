...
CREATE TABLE `tb_voucher_order`  (
  ...
  `pay_type` tinyint(1) UNSIGNED NULL DEFAULT NULL COMMENT '支付方式 1：余额支付；2：支付宝；3：微信',
  `status` tinyint(1) UNSIGNED NOT NULL DEFAULT 1 COMMENT '订单状态，1：未支付；2：已支付；3：已核销；4：已取消；5：退款中；6：已退款',
  `active_flag` tinyint(1) NULL DEFAULT 1 COMMENT '有效订单唯一约束标记，终态为NULL',
  `amount` bigint(20) NULL DEFAULT NULL COMMENT '下单金额快照（分）',
  ...
  `update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`) USING BTREE,
  KEY `idx_user_status_time` (`user_id`, `status`, `create_time`) USING BTREE,
  KEY `idx_user_voucher_status` (`user_id`, `voucher_id`, `status`) USING BTREE,
  UNIQUE KEY `uk_active_user_voucher` (`user_id`, `voucher_id`, `active_flag`) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci ROW_FORMAT = Compact;
